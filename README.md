...
`duckhouse` を起動すると、カレントディレクトリに `.duckhouse` というディレクトリを作成し、
...
|           Name            |             Description                    |
|---------------------------|--------------------------------------------|
| `home_directory`          | `./.duckhouse`。引数`-db.homedir`で設定可  |
...
< Duckhouse-Authnid: bar
< Duckhouse-Connectionid: C_c650b7ea
< Duckhouse-Duration: 2.1053ms
...
< HTTP/1.1 200 OK
< Content-Type: text/csv
< Duckhouse-Authnid: token2
< Duckhouse-Connectionid: C_a2551a00
< Duckhouse-Duration: 0s
...
```console
$ ./duckhouse -authnfile ./testdata/authn.json
